...
// Check for existing required metastrings for user registration, create them if necessary
    ...
    if (!$icontime_id = get_metastring_id('icontime')) $icontime_id=add_metastring('icontime');
    $icontime_value = time();
    if (!$icontime_value_id = get_metastring_id($icontime_value)) $icontime_value_id = add_metastring($icontime_value);
...
// Generate users with bulk database inserts
    ...
    for ($i = 1; $i <= $num_of_users; $i++){
        ...
        $createMetadataStatement.= '(' . ($lastEntityId+$i) . ',' . $icontime_id. ',' . $icontime_value_id. ',"integer",' . $owner_guid. ',' . $access_id. ',' . $action_time. ',"yes"),';
...
// Start icon generation
        ...
        for ($i=1; $i<=$num_of_users; $i++) {
            $fileIndex=mt_rand(0, count($iconFiles) - 1);
            ...
            $filehandler->setFilename("profile/" . ($lastEntityId + $i) . "large.jpg");
            $filehandler->open("write");
            $filehandler->write($iconImages[$fileIndex]['large']);
            $filehandler->close();
            $filehandler->setFilename("profile/" . ($lastEntityId + $i) . "medium.jpg");
            $filehandler->open("write");
            $filehandler->write($iconImages[$fileIndex]['medium']);
            $filehandler->close();
            $filehandler->setFilename("profile/" . ($lastEntityId + $i) . "small.jpg");
            $filehandler->open("write");
            $filehandler->write($iconImages[$fileIndex]['small']);
            $filehandler->close();
            $filehandler->setFilename("profile/" . ($lastEntityId + $i) . "tiny.jpg");
            $filehandler->open("write");
            $filehandler->write($iconImages[$fileIndex]['tiny']);
            $filehandler->close();
            $filehandler->setFilename("profile/" . ($lastEntityId + $i) . "topbar.jpg");
            $filehandler->open("write");
            $filehandler->write($iconImages[$fileIndex]['topbar']);
            $filehandler->close();
            $filehandler->setFilename("profile/" . ($lastEntityId + $i) . "master.jpg");
            $filehandler->open("write");
